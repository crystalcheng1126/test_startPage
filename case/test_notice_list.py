...
class TestNoticeList():
    ...
    def test_delete_notice(self,driver,host):
        '''测试删除公告'''
        list = self.enter_in_notice_page(driver, host, "d43c2133-a058-487d-b271-ade608548bfb",
                                         "68dc0e06-82b7-4024-9a5b-ae921cc53914")
        count_before=int(list.get_all_notice_count())#获取删除前的count数
        title=list.get_title_by_index(0)#获取第一条公告的标题
        list.delete_notice_by_index(0)#删除第一条公告
        assert list.get_toast_text()=="删除成功"
        count_after = int(list.get_all_notice_count() ) # 获取删除后的count数
        assert count_before-count_after==1
        assert list.get_title_index(title)==None#检查该标题不在公告列表中
