...
class NoticeListPage(Base):
    ...
    def delete_notice_by_index(self,index):
        '''删除指定索引的公告'''
        self.find_elements(self.delete_btns_loc)[index].click()
        self.click(self.confirm_btn_loc)

    def get_all_notice_count(self):
        '''获取公告总数'''
        time.sleep(3)
        return self.find_element(self.notice_count_text).text.split("当前总共")[1].split("条")[0]
